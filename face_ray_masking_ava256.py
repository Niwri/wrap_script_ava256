...
def _get_face_mask(image_bgr: np.ndarray) -> np.ndarray:
    """Exact port of wrap_script/face_ray_masking_wrapping_test.py's
    per-camera segmentation step: U2Net's whole-person mask picks a reliable
    SAM point-prompt (largest person blob's centroid, image-center fallback
    when no person blob is found), then SAM produces the actual face mask
    from that point. Both stages are fed the raw BGR image directly, no
    BGR->RGB conversion -- matching the reference exactly."""
...
def compute_face_ray_mask(
    ...
    camera_weights.update({c: 1.5 for c in (right_cameras | left_cameras)})
    ...
    for cam_id in camera_ids:
        ...
        seg_input = cv2.resize(image, (max(1, orig_w // 2), max(1, orig_h // 2)), interpolation=cv2.INTER_AREA)

        mask = _get_face_mask(seg_input).astype(np.uint8)
    ...
    face_angle_threshold_rad = math.radians(camera_selection.DEFAULT_ANGLE_THRESHOLD_DEG)
